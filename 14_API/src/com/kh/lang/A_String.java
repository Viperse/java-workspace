...
//		a.method4();
	...
	/*
	 * String 클래스
	 * - 블변의 클래스 : 객체를 생성하면 변경할 수 없다.
	 * - 변경이 적고 읽기가 많은 경우에 사용하면 좋다.
	...
	public void method1() {
		...
		// toString() 메소드는 String 클래스에서 오버라이딩
		...
		// equals() 메소드는 String 클래스에서 오버라이딩
		...
		// hashCode() 메소드는 String 클래스에서 오버라이딩
		...
		// 실제 주소값 : System.indentityHashCode() = 객체의 고유한 HashCode를 리턴하는 메소드
	...
	public void method2() {
		...
		// 2. concat(String str) : String - 전달받은 문자열과 원본 문자열을 합친 새로운 문자열을 생성해서 리턴
		...
		// substring(int beginIndex, int endIndex) : String - 문자열의 beginIndex 위치에서부터 endIndex-1 까지의 새로운 문자열을 새로 생성해서 리턴
		...
		//    indexOf(char ch) : int - 전달받은 ch의 인덱스를 리턴 (주어진 문자가 포함되어 있지 않으면 -1 리턴)
		...
		// 7. trim() : String - 문자열의 앞뒤 공백을 제거한 새로운 문자열을 생성해서 리턴
		str = "                   Hello, World!          ";
		...
		// 8. toCharArray() : char[] - 문자열의 문자들을 문자 배열에 담아서 해당 배열의 주소값 리터
		char[] arr = str.toCharArray();
		System.out.println(Arrays.toString(arr));
		
		// 9. static valueOf([기본 자료형, 문자 배열, 객체) : String - 정적 메소드는 매개변수로 입력받는 값들을 문자열로 변경해서 리턴
	 ...
	 * - StringBuilder와 StringBuffer의 차이점 : 동기화가 되느냐 안 되느냐
	...
	public void method3() {
		...
		// StringBuilder 클래스의 생성자에 정수값을 전달해 주면 해당 크기 만큼 버퍼가 생성
		StringBuilder sb2 = new StringBuilder(30);
		System.out.println("sb2의 문자열 길이 : " + sb2.length());  // 0
		System.out.println("sb2의 버퍼의 크기 : " + sb2.capacity());  // 30
		
		// StringBuilder 클래스의 생성자에 문자열을 전달해 주면 문자열 크기 + 16 만큼의 버퍼가 생성
	...
	public void method4() {
		...
		// 1. append(String str) : StringBuilder - 기존 문자열 뒤에 문자열을 추가
		sb.append("World!");
		...
		// 3. delete(int start, int end) : StringBuilder - start에서 end-1까지의 인덱스에 해당하는 문자열을 삭제
		sb.delete(1, 6);

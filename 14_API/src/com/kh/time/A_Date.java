...
//		a.method3();
	 ...
	 * - Date의 메소드는 대부분 deprecated(비권장) 되어있지만 여전히 사용
	 */
	...
	public void method1() {
		...
		// 1970년 1월 1일 00시 -> 한국 표준 시간(KST)으로 출력될 때는 그리니치 평균시(GMT) 보다 +9 증가된 시간으로 표시
		...
		SimpleDateFormat sdfo = new SimpleDateFormat("yyyy년 MM월 dd일 (E) a hh시 mm분 ss초");
		String date = sdfo.format(today);
...
//		Calendar cal = new Calendar();  추상 클래스는 객체 생성 X
		...
		System.out.println(today);
		today = new GregorianCalendar();  // 태국은 BuddhistCalender
		System.out.println(today);
		
		System.out.println();
		
		System.out.println("YEAR : " + today.get(Calendar.YEAR) + "년");
		System.out.println("MONTH : " + (today.get(Calendar.MONTH)+1) + "월");  // 0~11, 0 : 1월
		...
		System.out.println("SECONDE : " + today.get(Calendar.SECOND) + "초");
	...
	public void practice() {
		...
		try {
			...
			System.out.print("태어난 날을 입력해 주세요. (yyyy/MM/dd) : ");
			...
			String result1 = sdf.format(date.getTime());
			
			long timeInMillis = today1.getTimeInMillis() - date.getTimeInMillis();
			long result2 = ((((timeInMillis/1000)/60)/60)/24);
			System.out.println("태어난 요일은 " + result1 + "이고, 지금까지 " + result2 + "일이 지났습니다.");			

 ...
 * 
 * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 컬렉션(배열)의 저장 요소들을 하나씩 참조해서 람다식으로 처리할 수 있도록 해 주는 반복자
 * - 내부 반복자를 사용해서 중간 처리(intermediate), 최종 처리(terminal) 작업을 수행
 *   - 중간 처리 : 반복, 매핑(타입 변환), 필터링, 정렬 등
 *   - 최종 처리 : 반복, 카운팅, 평균, 총합 등의 집계 처리
 * - 중간 처리와 최종 처리 메소드를 구분하는 방법은 리턴 타입으로 알 수 있음!
 *   - 리턴 타입이 Stream이라면 중간 처리 메소드
 *   - 리턴 타입이 기본 타입이거나, Optional 객체라면 최종 처리 메소드
...
//		c.method2();
	...
	// 숫자 범위로 스트림을 생성하는 방법
	// - IntStream, LongStream, DoubleStream의 "range(), rangeClosed() 메소드"를 이용해서 생성
	public void method1() {

...
public class EmployeeController {
	...
	public Employee findeEmployeeByName(Employee[] ea, String name) {
		
		for(int i=0; i<ea.length; i++) {
			if (name.equals(ea[i].getName())) {
				return ea[i];
			}
		}
	...
	public int getAnnualSalary(Employee e) {
		...
		// instanceof 연산자 : 현재 참조변수가 실제로 어떤 클래스 형의 객체 주소를 참조하고 있는지 확인할 때 사용
		if(e instanceof Engineer) {
			Engineer eg = (Engineer) e; // 부모 -> 자식 : 강제 형 변환!
			return (e.getSalary() + eg.getBonus() ) * 12;
		}

...
//		f.method2();   // 다른 파일로 출력!
	...
	public void fileSave() {
		...
		try (FileWriter fw = new FileWriter(fileName, true)) {  // true 하면 새로운 게 계속 저장됨
			
			fw.write("IO 너무 어려워...\n");
			fw.write("너무 어려워요... \n");
			fw.write("왜 하는지 모르겠어요... \n");
	...
	public void method2() {
		 ...
		 * - 라인(line) 단위로 입출력이 편리
		 * - String readLine() : 한 라인만 읽어온다. (BufferedReader 메소드)
		 * - void newLine() : 라인 구분자(개행문자)를 출력한다. (BufferedWriter 메소드)
		 */
	...
	public void method3() {
		...
		// PrintWriter : 프린터와 유사하게 출력하는 print(), println() 메소드를 가지고 있는 보조 스트림
		try {
			br = new BufferedReader(new FileReader(fileName));
